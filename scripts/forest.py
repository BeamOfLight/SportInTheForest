...
def get_skills_resistance_data():
	...
	for location_id in xrange(1, get_locations_count() + 1):
		...
		data.append({
			...
			'label':'Сопротивление +{}'.format(str(resistance_new)),
			'duration': 0,
			'reuse': 50,
...
def get_skills_fitness_points_data():
	...
	for location_id in xrange(1, get_locations_count() + 1):
		...
		data.append({
			...
			'label':'Фитнес-очки +{}'.format(str(fitness_points_new)),
			'duration': 0,
			'reuse': 50,
			'specialisation': SPECIALISATION_NO,
			'extra_fitness_points_ratio1': 0,
...
def get_skills_multiplier_data():
	...
	for location_id in xrange(1, get_locations_count() + 1):
		...
		data.append({
			...
			'label':'Множитель +{}'.format(str(multiplier_new)),
			'duration': 0,
			'reuse': 50,
			'specialisation': SPECIALISATION_NO,
			'extra_fitness_points_ratio1': 0,
...
def get_skills_bonus_data():
	...
	for location_id in xrange(1, get_locations_count() + 1):
		...
		data.append({
			...
			'label': 'Множитель бонуса +{0:.2f} и шанс бонуса +{1:.1f}%'.format(bonus_multiplier_new, 100.0 * bonus_chance_new),
			'duration': 0,
			'reuse': 50,
			'specialisation': SPECIALISATION_NO,
			'extra_fitness_points_ratio1': 0,
...
def get_skills_active_self_restoration_data():
	...
	for required_level in [1, 5, 10, 14, 18, 21, 24, 27, 30, 32, 34, 36, 38, 40]:
		...
		data.append({
			...
			'label':'Восстановление своих ФО +{} + результат x {}'.format(str(value1_new), str(value2_new)),
			'duration': 1,
			'reuse': 50,
			'specialisation': specialisation,
			'extra_fitness_points_ratio1': 0,
...
def get_skills_active_my_team_restoration_data():
	...
	for required_level in [18, 21, 24, 27, 30, 32, 34, 36, 38, 40]:
		...
		data.append({
			...
			'label':'Восстановление ФО команды +{} + результат x {} '.format(str(value1_new), str(value2_new)),
			'duration': 1,
...
def get_skills_active_teammate_restoration_data():
	...
	for required_level in [14, 18, 21, 24, 27, 30, 32, 34, 36, 38]:
		...
		data.append({
			...
			'label':'Восстановление ФО союзника +{} + результат x {}'.format(str(value1_new), str(value2_new)),
			'duration': 1,
